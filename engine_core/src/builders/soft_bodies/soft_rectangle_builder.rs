...
impl SoftRectangleBuilder {
    ...
    /// Increase collision coverage by subdividing each edge into this many segments (>= 1)
    pub fn with_edge_subdivisions(mut self, segments_per_edge: usize) -> Self {
        self.edge_subdivisions = segments_per_edge.max(1);
        self
    }
    ...
    pub fn build(self, world: &mut PhysicsWorld) -> SoftRectangleHandle {
        ...
        // Track all nodes for potential collision/rendering management
        let mut nodes: Vec<RigidBodyHandle> = corners.to_vec();
        ...
        for (a, b) in edge_pairs {
            ...
            let len = delta.norm().max(1e-5);
            let n = self.edge_subdivisions.max(1);
            let seg_len = len / n as f32;
            let axis = delta / len; // normalized

            // Build chain of handles along this edge
            let mut prev = corners[a];
            if n > 1 {
                for k in 1..n {
                    ...
                    let mass = CircleRigidBodyBuilder::new(self.body_type, p, self.mass_radius)
                        .with_friction(self.friction);
                    let h = world.add_solid_body(mass);
                    nodes.push(h);
                    ...
                    .with_target_position(seg_len);
                    world.add_spring(spring, prev, h);

                    prev = h;
                }
            }

            // Connect last to corner b
            let spring = SpringBuilder::new_linear(
                axis,
                rapier2d::na::Point2::origin(),
                rapier2d::na::Point2::origin(),
            ...
            .with_target_position(seg_len);
            world.add_spring(spring, prev, corners[b]);
        }
        ...
        // Register for rendering/management
